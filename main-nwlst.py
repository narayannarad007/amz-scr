...
NUM_AT_ONCE = 10  # Lower for debugging
WAIT_TIME = 2.0
ERRORS = {
...
async def get_stuff_from_page(page, xpaths):
    ...
    for idx, xpath in enumerate(xpaths):
        try:
            print(f"  Trying XPath {idx+1}: {xpath[:60]}...")
            if "/@" in xpath:
                ...
                elem = await page.wait_for_selector(xpath, timeout=20000, state="attached")
                if elem:
                    ...
                    if not value.strip():
                        ...
                        value = ''.join([t for t in html if t not in ['<', '>']]).strip()  # Simple clean
                else:
                    value = None
            final_value = value.strip() if value else ERRORS["NO_DATA"]
            results.append(final_value if final_value else ERRORS["XPATH_NOT_FOUND"])
            print(f"  Got: {final_value[:50] if final_value else 'None'}")
        except Exception as e:
            print(f"  XPath {idx+1} failed: {str(e)[:100]}")
    ...
    async with async_playwright() as p:
        ...
        for start in range(0, len(urls), NUM_AT_ONCE):
            ...
            for result in results:
                ...
                sheet.append_row(row)  # Append even on failure – shows errors
                if result["ok"]:
                    done += 1
            for ctx in contexts:
                await ctx.close()
...
if __name__ == "__main__":
    ...
    print(f"Finished! Check sheet for {len(urls)} rows (including errors).")
